...
I'll use this block for creating helper functions:-) 
...
```{r}
file_name <- dir_ls(glob="NEW*Personality*.csv")
...
gender_ballons
```
There seems to be a much higher mean for the males, but also a very high standard deviation (which holds for both genders)

2. Is there a relation between sound level preference and which cola was chosen?
```{r}
sound_cola <- df %>% group_by(taste_cola) %>%
  summarise(avg_sound_pref=mean(sound_level_pref), sd_sound_pref=sd(sound_level_pref))
sound_cola
```
Given that the means are quite close and the sd is large-ish, there is no significant difference

3. Does handedness influence average tongue twisting speed?
```{r}
handedness_vs_tongue <- df %>% group_by(handedness) %>% summarise(mean(tongue_twist))
handedness_vs_tongue
```
It seems to be quite close for all 
...
```{r}
handedness_vs_tongue <- df %>% group_by(handedness) %>%
  summarise(mean_time=mean(tongue_twist), sd_tongue_twist=sd(tongue_twist), n_people=n())
handedness_vs_tongue

```
  
  3b. What does number of people tell us about the estimates of the mean? Are our results reliable?
      The results for right handed people are much more robust given the larger sample size. We can't really say anything
      about left-handed and nothing about ambidextrous people
...
ggplot(df, aes(x=handedness, y=shoesize, fill=handedness)) +
  ...
  scale_fill_manual(values = c("Ambidextrous" = "#E69F00", "Left-handed" = "#56B4E9", "Right-handed" = "#009E73")) + 
  labs(title="Shoesize vs Handedness", subtitle="No significant difference in shoesize (ambidextrous excluded)") +
  coord_cartesian(ylim = c(35, 45)) + 
  stat_compare_means(label.x="Left-handed", label.y=44) + 
  theme_minimal()
```
